...
WizzardInputOne::WizzardInputOne(QWidget* parent): QWizard(parent) {
    ...
    this->setPage(Page_Add_Soma, new AddSomaPage(neuron,this));
...
AddSomaPage::AddSomaPage( skelgenerator::Neuron*& neuron_,QWidget* parent ) : QWizardPage(parent), _neuron(neuron_)
{
  this->setTitle("Select Input VRML soma file");
  this->setSubTitle("Do you want to add a soma generated by Imaris?");

  radioYes = new QRadioButton("Yes");
  radioNo = new QRadioButton("No");

  selectSomaButton = new QPushButton( "Select File");
  somaPath = new QLineEdit();
  selectSomaButton->setEnabled( false);

  auto buttonLayout = new QHBoxLayout;
  buttonLayout->setSpacing(6);
  buttonLayout->addWidget( selectSomaButton);
  buttonLayout->addWidget(somaPath);
  ...
  connect(radioYes,&QRadioButton::toggled,this,[&](bool b){
    selectSomaButton->setEnabled( b);
    somaPath->setEnabled(b);
  });

  connect( selectSomaButton, &QPushButton::released, this, [&](){
    auto file = QFileDialog::getOpenFileName(this,tr("Select VRML longs file"),QString(),"Imaris(*.vrml *.VRML *.wrl *.WRL)");
    somaPath->setText(file);
  });

}

bool AddSomaPage::validatePage( )
{
  if (radioYes->isChecked())
  {
    //TODO checks
    QFuture<void> future = QtConcurrent::run( [ & ]( )
                                              { _neuron->addSoma( somaPath->text( ).toStdString( ) ); } );
    QFutureWatcher<void> watcher;
    QProgressDialog progress( this );
    connect( &watcher, SIGNAL( finished( ) ), &progress, SLOT( close( ) ) );
    watcher.setFuture( future );
    progress.setLabelText( "Processing soma file" );
...
int AddSomaPage::nextId( ) const
{
  if (!_neuron->hasFilamentSpines()) {
    return WizzardInputOne::Page_Add_Longs;
  }
  return WizzardInputOne::Page_Finish;
}
